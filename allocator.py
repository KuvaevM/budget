...
ACCESS_TOKEN_TG = config.get('TG', 'ACCESS_TOKEN_TG')
...
class current_day:
    day = int(str(datetime.now()).split('-')[2].split()[0])


class current_expense:
    expense = "food"


class current_change:
    change = "name"
...
def add_money(message):
    ...
    if not profile:
        ...
        return
    new_amount = profile.amount + amount
...
def choose_type(message):
    if message.text in ['food', 'taxes', 'fun', 'education', 'utilities']:
        ...
        current_expense.expense = message.text
...
def add_expenses(message):
    if 'exit' in current_expense.expense:
        main_keyboard(message)
        return
    ...
    change_data_in_costs(chat_id, current_expense.expense, expense)
...
def make_button_for_statistic():
    markup = telebot.types.ReplyKeyboardMarkup(True, True)
    button_name = telebot.types.KeyboardButton('all')
    button_amount = telebot.types.KeyboardButton('day')
    button_exit = telebot.types.KeyboardButton('exit')
...
def give_statistic(message):
    ...
    if not profile:
        ...
        markup = make_button_for_statistic()
        next_message = bot.send_message(message.chat.id,
                                        f""" Пожалуйста, выберите какой тип статистики Вы хотите посмотреть""",
                                        reply_markup=markup)
        bot.register_next_step_handler(next_message, choose_option)


def choose_option(message):
    if message.text == 'all':
        give_statistic_all(message)
        return
    if message.text == 'day':
        give_statistic_day(message)
    if 'exit' in message.text:
        main_keyboard(message)
        return


def give_statistic_day(message):
    expenses = get_all_daily_expenses(message.chat.id)
    show_picture(message, expenses)


def give_statistic_all(message):
    expenses = get_all_expenses(message.chat.id)
    show_picture(message, expenses)


def show_picture(message, expenses):
    profile = get_profile(message.chat.id)
    cur_labels = [labels[i] for i in range(len(expenses)) if expenses[i] != 0]
    expenses = [_ for _ in expenses if _ != 0]

    if not expenses:
        bot.send_message(message.chat.id, 'Простите, но у Вас не было еще расходов!')
    else:
        fig, ax = plt.subplots()
        ax.pie(expenses, labels=cur_labels, autopct='%1.1f%%', shadow=True,
        ...
        bot.send_message(message.chat.id, f'''У вас осталось: {profile.amount}''')
    markup = make_button_for_statistic()
    next_message = bot.send_message(message.chat.id, "Если Вы закончили смотреть статистику, нажмите exit.\n"
                                                     "Иначе, выберите категорию", reply_markup=markup)
    bot.register_next_step_handler(next_message, choose_option)
...
def change_profile(message):
    if message.text in ['name', 'amount']:
        ...
        current_change.change = message.text
...
def change_data_in_profile_bot(message):
    markup = make_button()
    if 'exit' in current_change.change:
        main_keyboard(message)
        return
    new_sum = message.text
    if current_change.change == "amount" and not new_sum.isdigit():
        next_message = bot.send_message(message.chat.id, 'Вы ввели не число, пожалуйста, напишите число!',
                                        reply_markup=markup)
        bot.register_next_step_handler(next_message, change_profile)
        return
    change_data_in_profile(message.chat.id, current_change.change, new_sum)
...
def bot_telegram_polling():
    while True:
        if current_day.day != int(str(datetime.now()).split('-')[2].split()[0]):
            current_day.day = int(str(datetime.now()).split('-')[2].split()[0])
        try:
...
if __name__ == '__main__':
    ...
    Thread(target=date_check).start()
